#!/usr/bin/python3
import os
os.environ['ONLINE_REPO']='/g100_work/OGS23_PRACE_IT/gocchipinti/OLD/FLOAT_GUIDO'
import sys
import ctypes as ct
import numpy as np
import datetime
...
#create directory to save files for gotm
if not os.path.exists(float_name):
    os.makedirs('/g100_scratch/userexternal/gocchipi/OASIM_ATM/builds/release_gcc/test/python/'+float_name)

def float_trajectory(float_name, datestart, dateend):
    '''
    Function to retrieve the timeseries and coordinates of ARGO-Float sampling
    Inputs:
      - float_name: name of the float of interest, dtype=str
      - datestart:  date since the data are retrieved, dtype=str
      - dateend:    date up to the data are retrieved, dtype=str
    Outputs: FLOATlist, FLOATlon, FLOATlat, FLOATtimes
      - FLOATlist = np.array(float name) # it is possible to check the correct float was studied
      - FLOATlon = array of the longitudes visited by the float
      - FLOATlat = array of the latitudes visited by the float
      - FLOATtimes = array of the dates of coordinates above
    '''
    import subprocess
...
#   var_mod = 'N3n'
    ...
    #dictionaries with float coordinates and times
    FLOATlon = {}  
    FLOATlat = {}
    FLOATtimes = {}
    for ff in FLOATlist:
    ...
    return FLOATlist, np.array(FLOATlon[float_name]), np.array(FLOATlat[float_name]), np.array(FLOATtimes[float_name])
...
OUT_Timelist = DL.getTimeList(datestart+"-00:00:00",dateend+"-00:00:00", seconds=dtnew) #datelist to be interpolated
...
FLOATlist, FLOATlon, FLOATlat, FLOATtimes = float_trajectory(float_name, datestart, dateend)


#create a list of coordinates with the same time spacing as OUT_Timelist
# between two consecutive observations of the argo float is taken the last seen position until the next one is observed, so the argo is jumping bewteen these positions
new_FLOATlon = np.zeros(len(OUT_Timelist))
new_FLOATlat = np.zeros(len(OUT_Timelist))
for ii,idate in enumerate(OUT_Timelist):
    for jj,jdate in enumerate(FLOATtimes):
        if idate <= jdate:
            new_FLOATlon[ii] = FLOATlon[jj]
            new_FLOATlat[ii] = FLOATlat[jj]
            break
        if jj == len(FLOATtimes)-1:
            new_FLOATlon[ii] = FLOATlon[jj]
            new_FLOATlat[ii] = FLOATlat[jj]


print('coordinates of float obtained',flush=True)
...
for it,tt in enumerate(OUT_Timelist[:]):
   i = it % nranks
   if i == rank :
       print('doing '+str(it)+' of '+str(len(OUT_Timelist)),flush=True)
       #get coordinates of the flot at time tt
       lat = np.array([new_FLOATlat[it]], dtype=np.float64, order='F')
       lon = np.array([new_FLOATlon[it]], dtype=np.float64, order='F') 
       olib = oasim_lib("../../OASIMlib/liboasim-py.so", "config.yaml", lat, lon)
       cunit = calc_unit(1, olib)
...
if rank == 0 :
    for iw,w in enumerate(wl):
        d = {'Date':ttlist, 'Ed'+str(w): edout[:,iw]}
        outfile=float_name+'/Ed_'+str(w)+'.txt'
        df=pd.DataFrame(d)
        df.to_csv(outfile, index=False, header=False, sep="\t")
    for iw,w in enumerate(wl):
        d = {'Date':ttlist, 'Es'+str(w): edout[:,iw]}
        outfile=float_name+'/Es_'+str(w)+'.txt'
        df=pd.DataFrame(d)
        df.to_csv(outfile, index=False, header=False, sep="\t")
